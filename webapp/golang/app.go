...
func dbInitialize() {
	sqls := []string{
		...
		"DELETE FROM banned_user_posts WHERE id > 10000",
...
var userRe = regexp.MustCompile(`\A[0-9a-zA-Z_]{3,}\z`)
var passwordRe = regexp.MustCompile(`\A[0-9a-zA-Z_]{6,}\z`)

func validateUser(accountName, password string) bool {
	return userRe.MatchString(accountName) && passwordRe.MatchString(password)
}
...
var loginTmpl *template.Template = template.Must(template.ParseFiles(
...
var indexTmpl *template.Template = template.Must(template.New("layout.html").Funcs(fmap).ParseFiles(
	getTemplPath("layout.html"),
...
var accountTmpl = template.Must(template.New("layout.html").Funcs(fmap).ParseFiles(
	getTemplPath("layout.html"),
...
var postsTmpl = template.Must(template.New("posts.html").Funcs(fmap).ParseFiles(
	getTemplPath("posts.html"),
	getTemplPath("post.html"),
))
...
var postsIDtmpl = template.Must(template.New("layout.html").Funcs(fmap).ParseFiles(
	getTemplPath("layout.html"),
...
func postAdminBanned(w http.ResponseWriter, r *http.Request) {
	...
	for _, id := range r.Form["uid[]"] {
		db.Exec(query, 1, id)
		db.Exec("INSERT INTO banned_user_posts SELECT * FROM posts WHERE user_id = ?", id)
